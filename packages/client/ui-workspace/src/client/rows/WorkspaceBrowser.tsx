...
function SessionTree({
  ...
  const renderGroup = (group: GroupNode, depth: number): ReactNode => {
    ...
    // fork: 置顶区是独立的一节，不是 Workspace：没有重命名/删除菜单，
    // 也不参与 Workspace 拖拽，只画表头与置顶的会话行。
...
function FlatList({
  ...
  pinnedSessionIds, onSessionPinned, usePanelInfo, setSessionOrder,
...
> & {
  ...
  const statuses = useSessionStatus(s => s)
  const rows = useMemo(
    () => deriveFlat(list, sessionIds, statuses, pinnedSessionIds),
