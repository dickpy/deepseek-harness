...
describe('DocumentTitle', () => {
  it('projects a durable title and restores the product title', () => {
    ...
    const mounted = render(<DocumentTitle {...props} productTitle="维小智" />)
    expect(document.title).toBe('维小智')
    act(() => { sessions.update((state) => { state.byId[sessionId]!.title = 'First title' }) })
    expect(document.title).toBe('First title — 维小智')
    act(() => { sessions.update((state) => { state.byId[sessionId]!.title = 'Revised title' }) })
    expect(document.title).toBe('Revised title — 维小智')
    act(() => {
