...
/**
 * 提供 `dsh-app://shell` 下的壳自有页面（登录页），只读 renderer 目录内的文件。
 * @param request - 壳文档请求。
 * @returns 文件响应，或 400/403/404/405。
 */
...
async function main(): Promise<void> {
  ...
  // fork: 门禁期间的协议面。登录页资产走 dsh-app://shell，后端就绪前 dsh-app://app
  // 一律 503（稍后由 appRequest 接管）；注册必须早于登录窗加载。
  ...
  // fork: 登录窗在门禁阶段就会经 preload 请求 locale 与产品版本，此时其余 IPC 面尚未注册。
  ...
  // fork: 协议处理器已在门禁前注册（登录窗需要 dsh-app://shell），这里只接管 app 分支。
  appRequest = (request) => {
    const url = new URL(request.url)
  ...
  app.setAboutPanelOptions({
    // fork: 品牌名固定为产品名，不取 app.getName()（asar 内会读到 scoped 包名）。
    applicationName: '维小智',
